...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        # or all can to the functions will endlessly
        # grow the same dict
    ...
    # If space appears before new line, we have a keyword.

    # Base case
    # =========
    # If newline appears first (or there's no space at all, in which
    # case find returns -1), we assume a blank line.
    # A blank line means the remainer of the data is the message.
    if (spc < 0) or (nl < spc):
        assert (nl == start)
        dct[b''] = raw[start+1:]
        return dct
